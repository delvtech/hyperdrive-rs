...
// TODO: The integer macros in this file can be improved in two important ways:
//
...
macro_rules! uint256 {
    ($number:expr) => {{
        ...
        $crate::utils::string_to_u256(str)
    }};
}

pub fn string_to_i256(s: &str) -> I256 {
    // Parse a string into a mantissa and an exponent. The U256 arithmetic
    // will overflow if the mantissa or the exponent are too large.
...
macro_rules! int256 {
    ($number:expr) => {{
        let str = stringify!($number);
        $crate::utils::string_to_i256(str)
    }};
}
